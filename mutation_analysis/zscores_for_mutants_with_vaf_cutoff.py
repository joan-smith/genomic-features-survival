...
def calculate_cox(mutation, clinical_data, key, outdir, metagene_file=None):
  df, clinical_data_with_sequenced_patients, num_patients = prep_data(mutation, clinical_data, key)
  df = df[df['VAF'] >= VARIANT_ALLELE_FREQ_CUTOFF]
  ...
  if argv is None:
    ...
    if cancer_type in key.index:
      ...
      if not os.path.isdir(outdir):
        os.makedirs(outdir)
      calculate_cox(mutation, clinical_data, key, outdir, metagene_file=metagene_file)
