...
Patients who have a mutation in any of the list of codons are a "1."
...
def calculate_cox_for_cancer_type(requested_data, mutation_data, outdir):
  ...
  output_data = []
  for i, request in requested_data.iterrows():
    gene = request.gene[1:]
    if gene in patients_with_gene.groups.keys():
      patients_with_requested_gene = patients_with_gene.get_group(gene)
      mutated_at_positions = patients_with_requested_gene[start_pos].isin(request.positions)
      ...
      if len(ids_with_requested_positions) >= MUTATION_PERCENT*clinical_with_sequenced_patients.shape[0]:
        ...
        outdict['gene'] = request.gene
        outdict['chromosome'] = request.chromosome
        outdict['gene_and_codon'] = request.gene_and_codon
        outdict['count'] = request['count']
        output_data.append(outdict)
  outdata = pd.DataFrame(output_data)
  print outdata
  if len(outdata):
    outdata = outdata.set_index(['gene_and_codon', 'gene', 'chromosome', 'count'])
  return outdata
